...
- **macOS**: `~/Library/Application Support/ai.unburdn.voice-agent/`
- **Windows**: `C:\Users\{username}\AppData\Roaming\ai.unburdn.voice-agent\`
- **Linux**: `~/.config/ai.unburdn.voice-agent/`
...
```bash
# macOS/Linux
mkdir -p ~/Library/Application\ Support/ai.unburdn.voice-agent/models

# Windows (PowerShell)
New-Item -ItemType Directory -Force -Path "$env:APPDATA\ai.unburdn.voice-agent\models"
```
