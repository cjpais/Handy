...
- The recording overlay is disabled by default on Linux (`Overlay: None`) because certain compositors treat it as the active window. When the overlay is visible it can steal focus, which prevents Handy from pasting back into the application that triggered transcription. If you enable the overlay anyway, be aware that clipboard-based pasting might fail or end up in the wrong window.
...
- **Solution:** Open **Settings > Advanced** and set **"Overlay"** to **"None"** to disable the overlay
