// Conditionally compile Apple Intelligence support
// When SKIP_APPLE_INTELLIGENCE env var is set at build time, stub implementations are used

#[cfg(all(
    target_os = "macos",
    target_arch = "aarch64",
    not(skip_apple_intelligence)
))]
mod native {
...
// Stub implementations when Apple Intelligence bridge is skipped or not on macOS arm64
#[cfg(not(all(
    target_os = "macos",
    target_arch = "aarch64",
    not(skip_apple_intelligence)
...
mod native {
    ...
    pub fn process_text(_prompt: &str, _max_tokens: i32) -> Result<String, String> {
        Err("Apple Intelligence not available".to_string())
    }
}
