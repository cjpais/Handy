...
#[command(
    name = "benchmark",
    about = "Benchmark ASR models on the Malayalam dataset"
)]
struct Cli {
...
fn main() -> Result<()> {
    ...
    // Load transcripts
    let transcript_str =
        std::fs::read_to_string(&cli.transcripts).context("Failed to read transcripts.json")?;
    let db: TranscriptDb =
    ...
    if cli.max_clips > 0 {
        ...
        eprintln!(
            "[benchmark] max_clips={} → {} WAV files after filtering.",
            cli.max_clips,
            wavs.len()
        );
    } else {
    ...
    for wav_path in &wavs {
        ...
        let entry = match db.get(&stem) {
            ...
            None => {
                eprintln!(
                    "[benchmark] WARNING: no transcript for '{}', skipping.",
                    stem
                );
                continue;
            }
        ...
        let noise_label = if path_lc.contains("noisy") {
            "noisy"
        } else {
            "clean"
        }
        ...
        match run_clip(
            ...
            refresh_kind,
            &stem,
            entry.is_multi_speaker,
            entry.is_code_switched,
            noise_label,
            &samples,
            audio_secs,
            &entry.clean_text,
    ...
    let clean_wavs: Vec<&PathBuf> = wavs
        .iter()
        .filter(|p| !p.to_string_lossy().to_lowercase().contains("noisy"))
        .collect();
    ...
    for wav_path in &clean_wavs {
        ...
        match run_clip(
            ...
            Ok(r) => results.push(r),
            Err(e) => eprintln!(
                "[benchmark] WARNING: heavy-noisy clip '{}' failed: {}",
                stem, e
            ),
        }
    }
    ...
    for &(label, is_cs, noise) in combined_groups {
        let matching: Vec<&ClipResult> = results
            .iter()
            .filter(|r| {
                !r.is_multi_speaker && r.is_code_switched == is_cs && r.noise_label == noise
            })
            .collect();
                    ...
                    match noise {
                        "noisy" => {
                            if let Some(nw) = wavs.iter().find(|p| {
                                ...
                                if let Ok((ns, nd)) = load_wav_16k(nw) {
                                    samps = ns;
                                    total_audio += nd;
                                }
                            } else {
                        ...
                        "heavy_noisy" => {
                            samps = apply_gaussian_noise(&samps, 0.20);
                            total_audio += dur;
                        }
                        _ => {
                            total_audio += dur;
        ...
        let hypothesis = match engine.transcribe(&combined_samples) {
            ...
            Err(e) => {
                eprintln!(
                    "[benchmark] WARNING: combined clip '{}' failed: {}",
                    label, e
                );
                continue;
            }
        ...
        let wer = if ref_words.is_empty() {
            ...
            (s + i + d) as f64 / ref_words.len() as f64
        };
        ...
        let cer = if ref_chars.is_empty() {
            ...
            (cs2 + ci2 + cd2) as f64 / ref_chars.len() as f64
        };
...
fn load_engine(cli: &Cli, whisper_lang: Option<String>) -> Result<BenchmarkEngine> {
    match &cli.model {
        ...
        ModelChoice::WhisperTurbo | ModelChoice::WhisperLarge => {
            ...
            eprintln!(
                "[benchmark] Loading Whisper from {:?} (GPU={}) …",
                bin_path, cli.use_gpu
            );
...
fn load_wav_16k(path: &Path) -> Result<(Vec<f32>, f64)> {
    ...
    let samples = if in_sr == target {
        mono
    } else {
        resample(&mono, in_sr, target)?
    };
...
fn apply_gaussian_noise(samples: &[f32], level: f32) -> Vec<f32> {
    ...
    while i < samples.len() {
        ...
        i += 1;
        if i < samples.len() {
            out.push((samples[i] + z1).clamp(-1.0, 1.0));
            i += 1;
        }
    }
    out
...
fn visit_dir(dir: &Path, out: &mut Vec<PathBuf>) -> Result<()> {
    for e in std::fs::read_dir(dir)? {
        ...
        if p.is_dir() {
            visit_dir(&p, out)?;
        } else if p.extension().and_then(|e| e.to_str()) == Some("wav") {
            out.push(p);
        }
    }
    Ok(())
...
impl GroupStats {
    ...
    fn rtf(&self) -> f64 {
        if self.total_audio == 0.0 {
            0.0
        } else {
            self.total_latency / self.total_audio
        }
    }
    fn avg_lat(&self) -> f64 {
        if self.count == 0 {
            ...
            self.total_latency * 1000.0 / self.count as f64
        }
    }
    fn avg_wer(&self) -> f64 {
        if self.count == 0 {
            ...
            self.total_wer / self.count as f64
        }
    }
    fn avg_cer(&self) -> f64 {
        if self.count == 0 {
            ...
            self.total_cer / self.count as f64
        }
    }
    fn avg_cpu(&self) -> f64 {
        if self.count == 0 {
            ...
            self.total_cpu / self.count as f64
        }
    }
}
...
fn print_report(results: &[ClipResult], combined: &[CombinedResult], model_label: &str) {
    ...
    let mut sum_dw1 = 0.0f64;
    let mut sum_dw2 = 0.0f64;
    let mut sum_dc1 = 0.0f64;
    let mut cnt = 0usize;
    ...
    println!("|-------|-------|----------|-----|-----|---|---|---|-----|---------|---------|");
    for c in combined {
        let rtf = if c.audio_secs == 0.0 {
            0.0
        } else {
            c.latency_secs / c.audio_secs
        };
        println!(
            "| {} | {} | {:.1} | {:.3} | {:.3} | {} | {} | {} | {:.3} | {:.0} | {:.1} |",
            c.label,
    ...
    let clean_clips: Vec<&ClipResult> = results
        .iter()
        .filter(|r| r.noise_label == "clean")
        .collect();
    let total_lat: f64 = clean_clips.iter().map(|r| r.latency_secs).sum();
    let total_dur: f64 = clean_clips.iter().map(|r| r.audio_secs).sum();
    let avg_wer: f64 =
        clean_clips.iter().map(|r| r.wer).sum::<f64>() / clean_clips.len().max(1) as f64;
    let avg_cer: f64 =
        clean_clips.iter().map(|r| r.cer).sum::<f64>() / clean_clips.len().max(1) as f64;
    let avg_lat: f64 = total_lat * 1000.0 / clean_clips.len().max(1) as f64;
    let rtf_clean = if total_dur == 0.0 {
        0.0
    } else {
        total_lat / total_dur
    };
    ...
    println!("|--------|-------|");
    println!(
        "| Overall WER (clean) | {:.3} ({:.1}%) |",
        avg_wer,
        avg_wer * 100.0
    );
    println!(
        "| Overall CER (clean) | {:.3} ({:.1}%) |",
        avg_cer,
        avg_cer * 100.0
    );
