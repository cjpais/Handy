...
#[tauri::command]
#[specta::specta]
pub fn cancel_benchmark(cancel_flag: State<'_, Arc<AtomicBool>>) {
...
pub async fn benchmark_ort_thread_count(
    ...
    // Save original thread count for RAII restore
    let original_thread_count = {
        let settings = crate::settings::get_settings(&app);
    ...
    let cancel_clone = cancel_flag.inner().clone();
    ...
    let result = tokio::task::spawn_blocking(move || {
        ...
        sorted_timings.sort_by_key(|(tc, _)| *tc);
        let all_timings = sorted_timings
            .iter()
