...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        // Colored theme uses green icons (for Linux)
        (AppTheme::Colored, TrayIconState::Idle) => "resources/uspeach.png",
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("Uspeach v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("Uspeach v{}", env!("CARGO_PKG_VERSION"))
    }
}
