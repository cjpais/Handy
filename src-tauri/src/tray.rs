...
pub enum AppTheme {
    Dark,
    Light,
    Colored, // Bundled colored theme for Linux
}
...
pub fn get_icon_path(theme: AppTheme, state: TrayIconState) -> &'static str {
    match (theme, state) {
        ...
        // Colored theme uses bundled icons (for Linux)
...
fn version_label() -> String {
    if cfg!(debug_assertions) {
        format!("unburdn. v{} (Dev)", env!("CARGO_PKG_VERSION"))
    } else {
        format!("unburdn. v{}", env!("CARGO_PKG_VERSION"))
    }
}
