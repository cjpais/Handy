...
| SETM-05 | P2 | mac | Debug mode is a saved setting; `--debug` is not ([Debug mode and hidden controls](../foundations/the-settings-model.md#debug-mode-and-hidden-controls)). | Defaults. | 1. Cmd+Shift+D, relaunch. 2. Turn it off, relaunch with `--debug`, relaunch again without. | Step 1: Debug section persists. Step 2: present only on the `--debug` run. | — |
