...
| WIN-05 | P2 | mac | Closing the window with Show Tray Icon off keeps the Dock icon, so the window can be reopened ([Edge cases](../foundations/windows-and-tray.md#edge-cases)). | Defaults. | 1. Turn Show Tray Icon off. 2. Close the window. 3. Look for Handy in the Dock. 4. Click the Dock icon. | The menu-bar icon is gone; the Dock icon stays; clicking it shows the window. | — |
