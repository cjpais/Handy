...
**Check for Updates.** Toggle, default on. Description: "Automatically check for new versions of Handy". Applied immediately: turning it off cancels any check in progress, changes the footer's update status to "Update Checking Disabled", and makes the tray's "Check for Updates…" item do nothing, because the window no longer listens for it; turning it on runs a check at once. See [Updates](../integration/updates.md).
