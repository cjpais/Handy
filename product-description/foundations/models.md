...
**Active.** The one downloaded model named in settings as the model to use. Badged "Active" and sorted first on the Models page, named in the footer, ticked in the tray. A fresh install has none until onboarding picks one. Deleting the active model from the Models page clears the selection. At launch and at every Rescan, a selection that names a model no longer in the list at all (a custom file or alternate quantization removed by hand) is cleared, and, once onboarding is complete, an empty selection is filled with the first downloaded model in catalog order without loading it. A catalog model whose file was removed by hand stays selected, because the catalog entry itself never disappears; the next dictation then fails with a load error (see [First launch](../setup/first-launch.md)). Selecting a model writes the setting first and then loads it; if the load fails the selection reverts to the previous model and a toast reads "Failed to load model: {name}".
