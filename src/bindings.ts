...
async changeMediaWhileRecordingModeSetting(mode: MediaWhileRecordingMode) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_media_while_recording_mode_setting", { mode }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
export type MediaWhileRecordingMode = "none" | "mute" | "pause" | "fade"
