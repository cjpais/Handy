...
async changeOrtThreadCountSetting(count: number) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_ort_thread_count_setting", { count }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async benchmarkOrtThreadCount() : Promise<Result<BenchmarkResult, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("benchmark_ort_thread_count") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async cancelBenchmark() : Promise<void> {
    await TAURI_INVOKE("cancel_benchmark");
}
}
