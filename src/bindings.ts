...
async changeWhisperRuntimeSetting(runtime: string) : Promise<Result<null, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("change_whisper_runtime_setting", { runtime }) };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
...
async downloadWhisperfileBinary() : Promise<Result<string, string>> {
    try {
    return { status: "ok", data: await TAURI_INVOKE("download_whisperfile_binary") };
} catch (e) {
    if(e instanceof Error) throw e;
    else return { status: "error", error: e  as any };
}
},
async isWhisperfileBinaryDownloaded() : Promise<boolean> {
    return await TAURI_INVOKE("is_whisperfile_binary_downloaded");
},
/**
...
export type WhisperRuntime = "whisper" | "whisperfile"
