...
interface TranslateTargetLanguageProps {
  descriptionMode?: "inline" | "tooltip";
  grouped?: boolean;
}

export const TranslateTargetLanguage: React.FC<
  TranslateTargetLanguageProps
> = ({ descriptionMode = "tooltip", grouped = false }) => {
  ...
  const selectedLanguage = getSetting("translate_target_language") || "en";
  ...
  const handleLanguageSelect = async (languageCode: string) => {
    await updateSetting("translate_target_language", languageCode);
    setIsOpen(false);
    setSearchQuery("");
  };

  const handleReset = async () => {
    await resetSetting("translate_target_language");
  };

  const handleToggle = () => {
    if (isUpdating("translate_target_language")) return;
    setIsOpen(!isOpen);
  };
  ...
  return (
    <SettingContainer
      title={t("settings.advanced.translateTargetLanguage.title")}
      description={t("settings.advanced.translateTargetLanguage.description")}
      ...
      <div className="flex items-center space-x-1">
        <div className="relative" ref={dropdownRef}>
          <button
            ...
            disabled={isUpdating("translate_target_language")}
          >
...
TranslateTargetLanguage.displayName = "TranslateTargetLanguage";
