    ...
    >
      <div className="flex items-center space-x-1">
        <div className="relative" ref={dropdownRef}>
          <button
            type="button"
            className={`px-2 py-1 text-sm font-semibold bg-muted/60 border border-input rounded min-w-[200px] text-start flex items-center justify-between transition-all duration-150 ${
              isUpdating("selected_language")
                ? "opacity-50 cursor-not-allowed"
                : "hover:bg-accent/10 cursor-pointer hover:border-ring"
            }`}
          ...
          {isOpen && !isUpdating("selected_language") && (
            <div className="absolute top-full left-0 right-0 mt-1 bg-background border border-input rounded shadow-lg z-50 max-h-60 overflow-hidden">
              {/* Search input */}
              <div className="p-2 border-b border-input">
                <input
                  ...
                  className="w-full px-2 py-1 text-sm bg-muted/60 border border-border rounded focus:outline-none focus:ring-1 focus:ring-ring focus:border-ring"
                />
              </div>
      ...
      {isUpdating("selected_language") && (
        <div className="absolute inset-0 bg-muted/60 rounded flex items-center justify-center">
          <div className="w-4 h-4 border-2 border-ring border-t-transparent rounded-full animate-spin"></div>
        </div>
      )}
    </SettingContainer>
  );
